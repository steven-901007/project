...
files = glob.glob('C:/Users/steve/Desktop/python相關資料/need data information/' + locate + '/' + year + '/need_information_set/*')
for file in files:

    day = file[84:94]
    # print(day)
    wb = load_workbook("C:/Users/steve/Desktop/python相關資料/need data information/" + locate + '/' + year + "/need_information_set/" + day+ ".xlsx")
    wsv = wb['V']
    wsu = wb['U']
    wsmv = wb['mv']
    wsmd = wb['md']
    ...
    wsu_10min.title = 'U'
    wsv_10min = wb_10min.create_sheet('V')
            ...
            try:
                ...
                for l in range(len(cu)):
                    try:
                        ...
                        tgmv = cmv[l]
                        tgmd = cmd[l]
                        if tgu >= meanu+1.5*sigmau or tgu <= meanu-1.5*sigmau:
                            cu.remove(tgu)
                        if tgv >= meanv+1.5*sigmav or tgv <= meanv-1.5*sigmav:
                            cv.remove(tgv)
    ...
    wb_10min.save("C:/Users/steve/Desktop/python相關資料/need data information/" + locate + "/" + year + "/10min/" + day + ".xlsx")
    print(day)
