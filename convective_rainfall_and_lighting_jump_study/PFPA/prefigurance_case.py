...
#取得對流性降雨station name
result = glob(f"{data_top_path}/rain_data/CR_{dis}km/{year}/{month}/**.csv")
...
# convective_rainfall_path =f"{data_top_path}/rain_data/對流性降雨36km/{year}/{month}/466880.csv"
    ...
    #取得周圍測站名稱
    around_station_names_df = pd.read_csv(f"{data_top_path}/rain_data/station_count_in_range/{year}_{month}/{station_name}.csv")
    ...
    for convective_rainfall_time in tqdm(convective_rainfall_times_df,desc=f"{station_name}資料建立中"):
        ...
        rainfall_path = f"{data_top_path}/rain_data/rainfall_data/{year}/{month}/{convective_rainfall_time_str}.csv"
