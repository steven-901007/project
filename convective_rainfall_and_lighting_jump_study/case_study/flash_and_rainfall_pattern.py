...
def flash_and_rainfall_pattern(year, month, day, time_start, time_end, dis, station_name, data_top_path, flash_source):
    ...
    shape_feature = ShapelyFeature(Reader(shp_path).geometries(), ccrs.PlateCarree(), edgecolor='black', facecolor="#0362025C")
    ...
    sc = ax.scatter(merged_df['lon'], merged_df['lat'], c=merged_df['count'], cmap=cmap, norm=norm, s=10, zorder=4, label='周圍測站')

    ## 畫 36km 藍圈
    circle = geodesic.Geodesic().circle(lon=point_lon, lat=point_lat, radius=dis * 1000)
    ax.plot(circle[:, 0], circle[:, 1], color='blue', transform=ccrs.PlateCarree(), label=f"{dis}km")
