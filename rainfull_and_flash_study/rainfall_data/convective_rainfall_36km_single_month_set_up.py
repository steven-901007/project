...
file_path = f"{data_top_path}/研究所/雨量資料/對流性降雨{dis}km/{year}/{month}"
...
# rain_data_path = result[0]
    ...
    for index, row in rain_datas.iterrows():
        ...
        if rain_data_rainfall >= 10:
            ...
            rain_data_station_path=  f"{data_top_path}/研究所/雨量資料/測站範圍內測站數/{year}_{month}/{rain_data_station_name}.csv"
...
for station in tqdm(range(len(rain_data_36km_station_list_list)),desc='資料建立'):
    if rain_data_36km_station_list_list[station] != []:
        ...
        save_path = f"{data_top_path}/研究所/雨量資料/對流性降雨{dis}km/{year}/{month}/{station_data_name[station]}.csv"
